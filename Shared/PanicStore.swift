...
class PanicStore: ObservableObject {
    ...
    private func loadVibrationSetting() {
        vibrationEnabled = userDefaults.object(forKey: vibrationEnabledKey) as? Bool ?? true
    }
    
    func saveVibrationSetting() {
        userDefaults.set(vibrationEnabled, forKey: vibrationEnabledKey)
    }
}
